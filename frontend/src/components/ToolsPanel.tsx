...
export default function ToolsPanel({
  ...
  const [runnerTab, setRunnerTab] = useState<RunnerTab>('manual')
  ...
  useEffect(() => {
    if (runnerTab === 'replay' && replayTaskId) {
      void loadTranscript(replayTaskId)
    }
  }, [runnerTab, replayTaskId, loadTranscript])
  ...
  }, [filtered.length, toolEvents.length])
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#0f0f15]">
      <div className="flex flex-col min-h-0 flex-[3] border-b border-cat-surface1">
        ...
        >
          {filtered.length === 0 && (
            <p className="text-cat-overlay text-center py-8">
              No tool executions yet. Run a sprint or use the test runner below.
            </p>
          )}
          {filtered.map((ev) => {
      ...
      <div className="flex flex-col min-h-0 flex-[2]">
        <div className="bg-cat-mantle border-b border-cat-surface1 px-4 py-2 flex items-center gap-2 shrink-0">
          ...
          <div className="flex gap-1 ml-2">
            {(['manual', 'replay'] as const).map((tab) => (
              <button
                key={tab}
                type="button"
                onClick={() => setRunnerTab(tab)}
        ...
        <div className="flex-1 overflow-y-auto p-3 text-[11px]">
          {runnerError && (
            <p className="text-rose-300 mb-2 text-[10px] font-mono">{runnerError}</p>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  className="bg-cat-base border border-cat-surface1 rounded px-2 py-1 text-cat-text"
                >
                  {AGENT_OPTIONS.map((a) => (
              ...
              <label className="flex flex-col gap-1 md:col-span-2">
                ...
                <input
                  ...
                  className="bg-cat-base border border-cat-surface1 rounded px-2 py-1 text-cat-text font-mono"
                />
              </label>
              <label className="flex flex-col gap-1 md:col-span-2">
                ...
                <textarea
                  ...
                  rows={5}
                  className="bg-cat-base border border-cat-surface1 rounded px-2 py-1 text-cat-text font-mono text-[10px]"
                />
              </label>
              <div className="md:col-span-2">
                <button
                  ...
                  className="px-4 py-1.5 rounded bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold"
                >
          ...
          {runnerTab === 'replay' && (
            <div className="space-y-3">
              ...
              {!loadingTranscript && transcriptEntries.length === 0 && replayTaskId && (
                <p className="text-cat-overlay text-[10px]">No tool calls in this task transcript.</p>
              )}
