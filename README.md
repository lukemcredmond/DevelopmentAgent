...
**Project-built skills:** When several skills overlap, select **2+** skills in the Add Skills modal and click **Build combined skill…** — including skills **already assigned** to the agent (use **Select all assigned**). There is no fixed limit on how many sources you can select; the backend merges in **chains of up to five skills per LLM call** (six or more sources run multiple rounds automatically and may take longer). Merge input uses each skill’s **workspace copy first**, then the global library (same order as the agent prompt), so edited workspace files and `built/…` project-only skills are included. The app uses your local Ollama model to dedupe and merge them into one markdown file under `workspace/skills/built/` (default name **`{agent}-combined.md`**, e.g. `dev-combined.md`; if that file already exists the next merge uses **`dev-combined-2.md`**, **`dev-combined-3.md`**, …). YAML frontmatter lists all source files. Review the preview, then **Save & assign** so the agent loads the merged skill from the workspace copy (workspace paths take precedence over the global library at prompt time). Optional **Unassign source skills** only removes those paths from the agent’s assignment list; **`global_skills/` is never deleted** (you can assign them again later). Unused **workspace** copies under `workspace/skills/` are removed when no agent still references that skill. Batch **Assign** without combining still attaches each file separately until the skills context budget truncates extras.
...
**Dev phase graph:** On Developer **In Progress** steps, AllHands runs Explore → Patch → Verify with hard tool budgets (defaults: 3 explore / 4 patch / 2 verify). **Done** means that step’s verify budget finished — not board/card Done; the next Developer step on the same card resets to Explore and shows a cycle / status line explaining why. When explore is spent without `apply_patch`/`write_file`, the step nudges once then stops (`explore_budget_exhausted`) instead of burning max LLM iterations on reads. Live stepper shows on the Agent Run bar and active board card; open the card’s **Phase graph** / **Flow** sections for Explore|Patch|Verify tool tags (synced). Setting: Workflow → Autonomy (`enableDevPhaseGraph`, `devExploreMaxTools`, …). Works on old cards the next time they run as Developer In Progress.
